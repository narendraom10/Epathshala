...
public partial class Admin_TeacherTrackRPT : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (ds_BMS.Tables.Count > ((int)EnumFile.AssignValue.Zero) && ds_BMS.Tables[0].Rows.Count > ((int)EnumFile.AssignValue.Zero))
            {
                ...
                ddlSchool.DataBind();
                ddlSchool.Items.Insert(((int)EnumFile.AssignValue.Zero), new ListItem("-- Select --"));
    ...
    protected void btnOk_Click(object sender, EventArgs e)
    {
        ...
        DataSet dsResult = obj_TracklogRPT.BAL_Select_TeacherTrackLog(Convert.ToInt32(ddlSchool.SelectedValue), Convert.ToInt32(ddlBoard.SelectedValue), Convert.ToInt16(ddlDivision.SelectedValue), Convert.ToDateTime(txtFdate.Text), Convert.ToDateTime(txtTodate.Text));

        if (dsResult.Tables.Count > 0 && dsResult.Tables[0].Rows.Count > 0)
        {
            dataDiv.Visible = true;

            lblActivityLogDate1.Text = txtFdate.Text + " To " + txtTodate.Text;
            ...
            ViewState["ActivityTable"] = dsResult.Tables[2];

            gvMaster.DataSource = dsResult.Tables[1];
            gvMaster.DataBind();
    ...
    protected void gvMaster_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        {
            Int64 SubjectID = Convert.ToInt64(DataBinder.Eval(e.Row.DataItem, "SubjectID"));
            GridView gvChild = (GridView)e.Row.FindControl("nestedGridView");

            DataTable dt = new DataTable();
            dt = (DataTable)ViewState["ActivityTable"];

            if (dt.Rows.Count > 0)
            {
                DataTable dtResult = dt.Clone();
                DataRow[] dr = dt.Select("SubjectID = " + SubjectID);
                foreach (DataRow drLoop in dr)
                {
                    dtResult.ImportRow(drLoop);
                }
