...
public partial class Report_ClassRoomWiseActivity : System.Web.UI.Page
{
    #region "Declaration"
    ...
    protected void btnGo_Click(object sender, EventArgs e)
    {
        ...
        DataSet dsResult = obj_TracklogRPT.BAL_Select_TeacherWiseBMSSCTWiseTrackLog(Convert.ToInt32(ddlSchool.SelectedValue), Convert.ToInt32(ddlBoard.SelectedValue), Convert.ToInt32(ddlMedium.SelectedValue), Convert.ToInt32(ddlStandard.SelectedValue), Convert.ToInt16(ddlDivision.SelectedValue), Convert.ToDateTime(txtFdate.Text), Convert.ToDateTime(txtTodate.Text));

        if (dsResult.Tables.Count > 0)
        {
            CommanCallUserControl(dsResult.Tables[0], "../ReportXMLFiles/ClasswiseActivityRPTEmpWise.xml", 1);
        }
        else
        {
    ...
    private void CommanCallUserControl(DataTable dt, string reporttype, int control)
    {
        ...
        try
        {
            ...
            else if (control == 2)
            {
                ...
                //div1.ConnectionString = connectionstring;


                ////reporttype = Server.MapPath("Files/MonthlySummary.xml");
                //div1.XMLReportFile = Server.MapPath(reporttype);
                ...
                //                    string str = xmllanguage.Tables[Session["LANG"].ToString().ToLower()].Rows[0][Columen].ToString();
    ...
    public void Displayselecteddata(Hashtable hashtable, object objsender)
    {
        if (ReportControl1.Visible)
        {
            ...
            tlblDateValue.Text = hashtable["ActivityDate"].ToString();
            ...
            if (dsResult.Tables.Count > 0)
            {
                ...
                CommanCallUserControl(dsResult.Tables[0], "../ReportXMLFiles/TeacherTrackRPTEmpWiseBMSSCT.xml", 2);
            }
            else
            {
                WebMsg.Show("No data found.");
