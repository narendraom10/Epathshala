...
public partial class Admin_SchoolDetail : System.Web.UI.Page
{
    #region "Declaration"
        ...
        if (!IsPostBack)
        {
            try
            {
                if ((Request.QueryString["RegID"] != "" || Request.QueryString["RegID"] != null))
                {
                    ...
                    dsSelect = obj_BAL_School.BAL_School_SelectAllOrBySchRegID("BySchRegID", Convert.ToInt32(Request.QueryString["RegID"].ToString()), SearchCondition);
                    if (dsSelect.Tables.Count > Convert.ToInt32(EnumFile.AssignValue.Zero))
                    {
                        ViewState["SchRegID"] = Request.QueryString["RegID"].ToString();
    ...
    {
        Page.RegisterClientScriptBlock("anything", "<script> window.opener.location = 'SchoolList.aspx'; window.close(); </script>");
    }
        ...
        foreach (GridViewRow gr in gvSchoolRegistrationBMSdetail.Rows)
        {
            ...
            if (chk.Checked == true)
            {
                ...
                if (StatusID == Convert.ToInt32(EnumFile.Status.Active) || StatusID == Convert.ToInt32(EnumFile.Status.Deactive))
                {
                    ...
                    {
                        tmp_SchoolBMSIDStr = gvSchoolRegistrationBMSdetail.DataKeys[gr.RowIndex]["SchoolBMSID"].ToString();
                    }
                    else
                    {
                        tmp_SchoolBMSIDStr = tmp_SchoolBMSIDStr + " , " + gvSchoolRegistrationBMSdetail.DataKeys[gr.RowIndex]["SchoolBMSID"].ToString();
                    }
    ...
    protected void GetBoards()
    {
        try
        {
            ...
            if (dsBoards.Tables.Count > 0)
            {
                ddlBoardI.Items.Clear();
