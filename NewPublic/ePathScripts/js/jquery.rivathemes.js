...
//					$wrapper.width(scrol - offsetLeft);
				...
				if (!isReadry)  {
					if ($this.hasClass('epath-section-projects')) {
						fixH = scrol * 0.6;
					}
				}
...
(function( $ ){
	$.fn.rivaSorting = function( options ) {
		var $this = this,
			...
			$filters = $this.find('.epath-sorting-filters'),
			$elems = $this.find('.epath-sorting-item');
		if (showAll) {
