    ...
    public class MultiCurrency
    {
        ...
        private string ReturnHashtableValue()
        {
            ...
            for (int i = (htPunctuation.Count - 1); i >= 0; i--)
            {
                ...
                {
                    if (GetWordConversion((htPunctuation[i + 1]).ToString()) != "")
                        strFinalString = strFinalString + GetWordConversion((htPunctuation[i + 1]).ToString()) + StaticPrefixFind((i+1).ToString());
                }
            }
